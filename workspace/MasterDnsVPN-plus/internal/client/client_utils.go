...
func (c *Client) ActiveListenPort() int {
	if c == nil || c.tcpListener == nil || len(c.tcpListener.listeners) == 0 {
		return 0
	}
	if addr, ok := c.tcpListener.listeners[0].Addr().(*net.TCPAddr); ok && addr != nil {
		return addr.Port
	}
	return 0
}
