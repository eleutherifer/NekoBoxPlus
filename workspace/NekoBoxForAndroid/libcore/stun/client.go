...
type Client struct {
	...
	softwareName                 string
	rfc3489Mode                  bool
	allowResponseAddressMismatch bool
	responseAddressMismatch      bool
	conn                         net.PacketConn
	tcpConn                      net.Conn
	tcpConnSet                   bool
...
// SetResponseAddressMismatchAllowed controls whether NAT discovery may continue
// when a transaction-ID-matched response comes from an unexpected transport
// address. The mismatch is still recorded for the caller to report.
func (c *Client) SetResponseAddressMismatchAllowed(allowed bool) {
	c.allowResponseAddressMismatch = allowed
}

// ResponseAddressMismatch reports whether an accepted response came from an
// unexpected transport address.
func (c *Client) ResponseAddressMismatch() bool {
	return c.responseAddressMismatch
}

func (c *Client) validateResponseAddress(matches bool) error {
	if matches {
		return nil
	}
	c.responseAddressMismatch = true
	if c.allowResponseAddressMismatch {
		return nil
	}
	return errors.New("server error: response IP/port")
}
